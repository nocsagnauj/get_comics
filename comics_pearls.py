...
if __name__ == "__main__":
    ...
    for year in range(2014, 2017):
        for month in range(1, 13):
            ...
            for d in days:
                if d > 0:
                    url_comic = SITE_PATH + comic_url_name + \
                        "/{0}/{1:02d}/{2:02d}".format(year, month, d)
                    print(url_comic)
                    ...
                    start_filename = header_html_img.find(
                        "\"", point_filename) + 1
                    end_filename = header_html_img.find("\"", start_filename)
                    full_filename = header_html_img[
